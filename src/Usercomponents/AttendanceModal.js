...
export default function AttendanceModal() {
  ...
  const params = useParams();
    console.log("params in attendence modal => ",params.id)
  let obj ="true"
  const [startDate, setStartDate] = React.useState();
  const [EndDate, setEndDate] = React.useState();

  const [Att, setUserAttendence] = React.useState([]);
  ...
  const handleClose = () =>{ 
    ...
    fetch(`http://localhost:8000/api/Attendence/userAttendence${params.id}`,requestOptions)
      .then(resp => resp.json())
      .then(resp => {
